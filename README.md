...
real-time Bluetooth stream to third-party apps — the only way to read live HR straight
off the ring is the reverse-engineered protocol in **Direct-from-ring (experimental)**
below, which has real caveats.
...
| --- | --- | --- |
| **Oura Ring — direct BLE** (experimental) | Near real-time | Reading live HR straight off the ring, no cloud |
...
### 📡 Direct-from-ring (experimental) — read this before using it

There's now an **Oura Ring (direct)** source that pulls live heart rate **straight off
the ring over Bluetooth, bypassing the cloud API entirely**. It speaks the ring's
own protocol: scan for the ring service, run its encrypted auth handshake, start the
real-time PPG stream, and decode inter-beat intervals (IBI) into BPM.

**This is a reverse-engineered, unofficial integration and comes with real strings
attached — be aware before relying on it:**

- **It needs your ring's private 16-byte auth key.** That key is generated by the
  *official* Oura app when you pair, and stored in that app's local database. A
  third-party app cannot mint it. You must **extract it from the official app**
  (rooted Android / jailbroken iOS / app-data dump, or a BLE capture during pairing)
  and paste the 32-hex-character key into **Connect → Oura Ring · direct**. It's kept
  in the Keychain.
- **It uses Oura's proprietary, undocumented protocol** (reconstructed from the
  community [`open_oura`](https://github.com/Th0rgal/open_oura) project). This is
  **against Oura's Terms of Service** and can **break on any firmware update**.
- **It's unverified.** The protocol constants (GATT UUIDs, auth flow, start/stop
  stream frames, IBI decoding) are implemented from the reverse-engineering notes but
  couldn't be tested here without a physical ring. The frame offsets are isolated in
  `OuraRingHeartRateSource.swift` and easy to tune if your ring/firmware differs.

If it won't connect, use a **Bluetooth strap** — that's the reliable path (and it's
what Oura's own app uses for live workout HR).
